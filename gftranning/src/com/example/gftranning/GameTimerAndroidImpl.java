...
public class GameTimerAndroidImpl extends GameTimer implements Handler.Callback {
    ...
    private final HashMap<Long, ArrayList<TimerEvent>> mTimerEvents = new HashMap<Long, ArrayList<TimerEvent>>();

    // each game id can only schedule one timer event
    @SuppressLint("UseSparseArrays")
    private final HashMap<Integer, TimerEvent> mGameIdToTimerEventMap = new HashMap<Integer, TimerEvent>();

    // all the timer callback will receive the same TimerEvent when the event's
    // time is coming
    private final ArrayList<GameTimerCallback> mCallbacks = new ArrayList<GameTimerCallback>();
    ...
    private static final String TAG = GameTimerAndroidImpl.class.getSimpleName();
                    ...
                    if (msg.obj != null) {
                        ...
                        if (tags != null) {
                            ...
                            for (TimerEvent aTag : tags) {
                                if (mGameIdToTimerEventMap.get(aTag.id) == null) {
                                    tags.remove(aTag);
                                    continue;
                                }
                                if (absTime.longValue() != mGameIdToTimerEventMap.get(aTag.id).absTime) {
                                    tags.remove(aTag);
                                } else {

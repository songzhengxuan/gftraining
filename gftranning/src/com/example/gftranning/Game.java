...
public class Game implements GameTimer.GameTimerCallback {
    ...
    public Game(long displayTime, long waitTime, int distance, GameTimer timer, ISequenceSource generator,
            IGameUI gameUI) {
        this.mId = 0;
    ...
    public void setForceRate(int forceRatePercent) {
        if (forceRatePercent < 0 || forceRatePercent > 100) {
            throw new IllegalAccessError("invalid forceRate:" + forceRatePercent);
        }
    ...
    public boolean checkAndRememberUserInput(boolean input) {
        ...
        if (ret) {
            ...
            mGameUI.onError(mResults.get(mResults.size() - 1), mResults.get(mResults.size() - 2 - mDistance));
        }
        rememberInput(input);
    ...
    private ResultAndInputStatus getResultAndInputMatchStatus() {
        if ((mInputs.size() + mDistance + 2) == mResults.size()) {
            ...
            throw new IllegalStateException("results " + mResults.size() + " and input " + mInputs.size()
                    + " number mismatch");
        }
    }
    ...
    public void onTimer(int id, int tag) {
        ...
        switch (tag) {
            case TIMER_EVENT_SHOW_START:
                ...
                mTimer.setNextTimerEventDelayed(mDisplayTime, mId, TIMER_EVENT_SHOW_END);
                break;
            case TIMER_EVENT_SHOW_END:
                ...
                mGameUI.hide();
                mTimer.setNextTimerEventDelayed(mWaitTime, mId, TIMER_EVENT_WAIT_END);
                break;
